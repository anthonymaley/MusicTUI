...
- Keys: `1/2/3/4/5/6` jump to a tab, `Tab`/`Shift-Tab` cycle, `[`/`]` switch Library sub-view or Radio Favorites/Live/Personal, `a` cycle Library Artists tier (All / 12″/EP / Albums) or open Radio's add-by-URL field, `↑↓` + `PgUp/PgDn/Home/End` navigate (Radio has no page/home/end jumps), `Space` play/pause, `</>` previous/next, `[ ]` seek (Now) / `←→` per-speaker volume (Speakers), `z` shuffle-play, `l` favorite (Now) / `f` favorite (Radio), `+/-` master volume, `n` next-up options, `/` filter (Playlists/Library) or search (Radio), `Esc` back, `q` or `Ctrl-C` quit (Ctrl-C quits even from inside a search or filter field).
...
- `music play --album "X"`, and `music play "X"` when X resolves to an album, play a bounded album: the CLI builds a temporary playlist, plays it, and a detached one shot helper deletes it once playback leaves it. The container appears in Music's sidebar for its duration (MusicTUI's Playlists rail hides it; Music.app shows every playlist). The album stops at its end only when Autoplay is off. Your shuffle setting is left alone, so with shuffle on the album plays shuffled within itself. If a container outlives its watcher, `music playlist cleanup` collects it. If X matches more than one distinct album (a broad query like "live", or two different artists' albums sharing a title), nothing plays: the CLI lists the matches and asks you to be more specific or add `--artist`, rather than guessing and building a container from the wrong one.
- Single-song play is bounded too and stops at the end of the song: `music play --song "X"`, `music play "Song" "Artist"`, `music play <N>` off a listing, and a bare `music play "X"` that resolves to a song. The container is seeded by the track's persistent ID and the CLI refuses to play a container it cannot confirm holds that track. A song that is not in the library is added from the catalog first; the added row is identified by comparing the library before and after and then by title, artist and album, and two indistinguishable new rows stop the command rather than producing a guess.
...
│   │       ├── LibraryDataSources.swift   # library browse: one AppleScript bulk read, grouped in Swift
...
Current version: v3.13.0. All five locations stay in sync:
