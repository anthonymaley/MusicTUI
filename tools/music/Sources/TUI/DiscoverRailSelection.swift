// The Discover tab's rail selection policy.
//
...
// title string.
//
// Shared with `music discover` on purpose: CLI/TUI drift on shared data is this
...
func discoverRailFreshness(_ rail: DiscoverRail) -> Int {
...
func discoverRailIsAllFromYear(_ rail: DiscoverRail, _ year: Int) -> Bool {
...
func selectDiscoverRails(_ rails: [DiscoverRail], currentYear: Int, slots: Int = 5) -> [DiscoverRail] {
    var pool = rails
    var picked: [DiscoverRail] = []

    func takeFirst(_ match: (DiscoverRail) -> Bool) {
    ...
    if picked.count < slots {
        ...
        let allCurrent = albums.first { discoverRailIsAllFromYear(pool[$0], currentYear) }
        let freshest = albums.sorted { a, b in
            let fa = discoverRailFreshness(pool[a]), fb = discoverRailFreshness(pool[b])
...
func discoverCurrentYear(_ now: Date = Date()) -> Int {
    Calendar.current.component(.year, from: now)
}

/// The single place a raw feed becomes Discover's curated rails. Both `music discover`
/// and the Discover tab call THIS, not the two functions it composes.
///
